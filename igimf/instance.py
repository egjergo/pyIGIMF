"""
Based on Gjergo, Kroupa, Yan, Sorokin, Zhang, Jerabkova, Zonoozi and Haghi (2023, ApJS)
...
class StellarIMF(Parameters):
    ...
    def __init__(self, M_ecl:float, metal_mass_fraction: float, SFR: float):
        super().__init__(metal_mass_fraction, SFR)
        self.M_ecl = M_ecl
        self.alpha_1 = float(self.alpha_1_func()) #float(self.alternative_alpha_1_func())
        self.alpha_2 = float(self.alpha_2_func())
        self.alpha_3 = self.alpha_3_func()
        self.stellar_IMF()
        self.IMF_func = np.vectorize(self.IMF_func)
        self.IMF_mass_weighted_func = np.vectorize(self.IMF_mass_weighted_func)
    ...
    def alpha1_arctan_func(self):
        return 1.3 + np.arctan(1.3e2*(self.metal_mass_fraction - 0.0134))/1.3
    def alternative_alpha_1_func(self, a1=1.3):
        Z = self.metal_mass_fraction
        maximum = 1.3*2
        Z_midpoint = self.solar_metallicity
    ...
    def stellar_IMF(self):
        ...
        self.IMF_mass_weighted_func = lambda m: util.mass_weighted_func(m, self.IMF_func)
...
class IGIMF(Parameters):
    ...
    def gwIMF(self):
        r"""Eq. (12)"""
        ecl_MF = ECMF(SFR=self.SFR, metal_mass_fraction=self.metal_mass_fraction)
...
def main():
    ...
    ecl_MF = ECMF(SFR=SFR,metal_mass_fraction=massfrac)
